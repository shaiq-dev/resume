...
aws cloudformation deploy --template-file ${STACK_TEMPLATE} --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM --stack-name ${STACK_NAME} --no-fail-on-empty-changeset --parameters ParameterKey=BucketName,ParameterValue=${S3_BUCKET} ParameterKey=LambdaCodeKey,ParameterValue=lambda/dresume.zip